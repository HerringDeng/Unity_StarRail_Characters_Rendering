Shader "Custom/HSR_Character_Shader"
{
    Properties
    {
        ...
        [Header(Outline)]
        [KeywordEnum(Off, Width_Mode, Pixel_Mode)]_Outline ("Outline off/on", float) = 0
        _OutlineColor ("Outline Color", Color) = (0, 0, 0, 0)
        _OutlineWidth ("Outline Width", float) = 0
        _OutlineZBias ("Outline Z Bias", float) = 0
        [Toggle]_DynamicOutlineWidth("Dynamic Outline Width", float) = 0
        _OutlineMinWidth("Outline Min Width", float) = 0
        _OutlineMaxWidth("Outline Max Width", float) = 0
    }
    SubShader
    {
        Tags 
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                half3 noseOutline = 0;
                #if _AREA_FACE
                    noseOutline = lerp(0, _OutlineColor.rgb, step(dot(headForward, viewDirWS), 1-lightMap.b));
                #endif
        ...
        Pass
        {
            ...
            HLSLPROGRAM
            #pragma target 2.0
            #pragma shader_feature_local _ _OUTLINE_OFF _OUTLINE_WIDTH_MODE _OUTLINE_PIXEL_MODE
            // -------------------------------------
            // Universal Pipeline keywords
            #pragma multi_compile _ _MAIN_LIGHT_SHADOWS _MAIN_LIGHT_SHADOWS_CASCADE _MAIN_LIGHT_SHADOWS_SCREEN
            #pragma multi_compile _ _ADDITIONAL_LIGHTS_VERTEX _ADDITIONAL_LIGHTS
            ...
            Varyings Vert(Attributes input)
            {
                ...
                outlinePostionWS += biasDir * _OutlineZBias;
                //法线
                ...
                //固定粗细描边模式
                #if _OUTLINE_WIDTH_MODE
                    float3 positionVS = TransformWorldToView(outlinePostionWS);
                    float3 normalVS = TransformWorldToViewDir(normalWS);
                    //恒定z值，可以把这个值改成变量，然后用Torus（在工程里面搜我准备好了）试一下
                    float outlineWidth = clamp(_OutlineWidth * cameraDistance, _OutlineMinWidth, _OutlineMaxWidth);
                ...
                //固定像素描边模式
                    ...
                    float outlineWidth = clamp(_OutlineWidth * cameraDistance, _OutlineMinWidth, _OutlineMaxWidth);
                    float2 outlineOffset =  (outlineWidth * output.outlinePostionHCS.w) / (_ScreenParams.xy / 2.0);
